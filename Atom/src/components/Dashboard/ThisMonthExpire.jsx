...
import { Edit, Trash2, Search, ChevronDown, MoreVertical, Download, Plus } from 'lucide-react';
...
const ThisMonthExpire = () => {
  ...
  // State for filters (simplified or same as AMC.jsx if needed)
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', 'expiring_amcs.xlsx');
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);

      toast.success('Expiring AMCs exported successfully.');
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col space-y-3 md:space-y-0 md:flex-row justify-between items-start md:items-center mb-4 md:mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">AMCs Expiring This Month</h1>
      ...
      {/* AMC Form Modal for Renew - Assuming AMCForm handles renewal with a prop */}
      {isRenewModalOpen && (
        <AMCForm
          isEdit={false} // Treat as create but prefill data for renewal
          isRenew={true} // Add a prop if AMCForm supports renewal logic
          initialData={{
            ...currentAMC,
            startDate: new Date(currentAMC.endDate), // Example: start renewal from end date
