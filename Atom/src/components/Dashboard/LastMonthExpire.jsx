...
const LastMonthExpire = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', 'last_month_expiring_amcs.xlsx');
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);

      toast.success('Last month expired AMCs exported successfully.');
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col space-y-3 md:space-y-0 md:flex-row justify-between items-start md:items-center mb-4 md:mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">AMCs Expired Last Month</h1>
