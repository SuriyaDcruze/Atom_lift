...
import AMCForm from '../Dashboard/Forms/AMCform'; // Assuming this is the path to AMCForm
import { Edit, Trash2, Search, ChevronDown, MoreVertical, Download } from 'lucide-react';
...
const NextMonthExpire = () => {
  ...
  const fetchData = async (retryCount = 2) => {
    ...
    try {
      ...
      const amcDataMapped = amcResponse.data
        .filter(item => {
          const endDate = new Date(item.end_date);
          return endDate.getMonth() === nextMonth && endDate.getFullYear() === nextMonthYear;
        })
        .map(item => ({
          id: item.id,
          amc: item.amc_name || 'N/A',
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', 'next_month_expiring_amcs.xlsx');
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);

      toast.success('Next month expiring AMCs exported successfully.');
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col space-y-3 md:space-y-0 md:flex-row justify-between items-start md:items-center mb-4 md:mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">AMCs Expiring Next Month</h1>
                    ...
                    <td className="p-3 lg:p-4 whitespace-nowrap">
                      <div className="flex space-x-2">
                        ...
                        <button
                          ...
                          className="bg-[#243158] text-white px-2 py-1 rounded-lg hover:bg-[#0f131d]"
                          title="Renew"
                          aria-label="Renew AMC"
                        >
                          Renew
                        </button>
                        <button
      ...
      {isEditModalOpen && (
        <AMCForm
          ...
          onSubmitSuccess={(message) => {
            fetchData();
            toast.success(message || 'AMC updated successfully!');
          }}
          onSubmitError={(error) => {
            toast.error(error || 'Failed to update AMC');
          }}
      ...
      {/* AMC Form Modal for Renew */}
      {isRenewModalOpen && (
        <AMCForm
          isEdit={false} // Treat as create but prefill data for renewal
          isRenew={true} // Assuming AMCForm supports renewal logic
          initialData={{
            ...currentAMC,
            startDate: new Date(currentAMC.endDate), // Start renewal from end date
            // Adjust other fields as needed for renewal
          }}
          onClose={() => {
          ...
          onSubmitSuccess={(message) => {
            fetchData();
            toast.success(message || 'AMC renewed successfully!');
          }}
          onSubmitError={(error) => {
            toast.error(error || 'Failed to renew AMC');
          }}
          apiBaseUrl={apiBaseUrl}
          dropdownOptions={{
            customerOptions: customerOptions.map(item => item.site_name),
